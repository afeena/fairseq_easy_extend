...
class SequenceGenerator(nn.Module):
    def __init__(
        ...
        self.model.set_normalized_tgt_embeddings()
        self.model.eval()
...
class EnsembleModel(nn.Module):
    ...
    def set_normalized_tgt_embeddings(self):
        for model in self.models:
            if hasattr(model, "set_normalized_tgt_embeddings"):
                model.set_normalized_tgt_embeddings()
    def forward(self):
        pass
    ...
    def build_index(self, metric=None):
        return self.single_model.build_index(metric)
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            probs = model.get_normalized_probs(
                decoder_out_tuple, log_probs=True, sample=None, metric=metric
            )
            probs = probs[:, -1, :]
